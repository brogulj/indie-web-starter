export const MusicReviewsTemplate = /* html */ `
<main class="mx-auto max-w-6xl p-2 md:p-4">
  <article class="h-entry overflow-hidden rounded-3xl border border-gray-300 bg-white">
    <header class="border-b border-gray-200 bg-gradient-to-r from-emerald-50 via-white to-cyan-50 p-5 md:p-7">
      <p class="text-xs uppercase tracking-[0.2em] text-emerald-700">Music Review</p>
      ...
      <div class="mt-3 flex flex-wrap items-center gap-2 text-sm text-gray-600">
        <a class="u-url rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 hover:bg-emerald-100" href="/music-reviews/{{slug}}">/music-reviews/{{slug}}</a>
    ...
    <div class="grid gap-6 p-5 md:grid-cols-3 md:p-7">
      <section class="space-y-4 md:col-span-1">
        ...
        <div class="rounded-2xl border border-gray-200 bg-gray-50 p-4 text-sm text-gray-700">
          <p><strong>Type:</strong> {{data.releaseType}}{{^data.releaseType}}-{{/data.releaseType}}</p>
          <p class="mt-1"><strong>Release Date:</strong> {{data.releaseDate}}{{^data.releaseDate}}-{{/data.releaseDate}}</p>
      ...
      <section class="space-y-5 md:col-span-2">
        <section class="rounded-2xl border border-gray-200 bg-gray-50 p-4">
          <div class="grid gap-2 text-sm text-gray-700 sm:grid-cols-2">
            <p><strong>Artist:</strong> {{data.artistName}}{{^data.artistName}}-{{/data.artistName}}</p>
            <p><strong>Release:</strong> {{data.releaseTitle}}{{^data.releaseTitle}}-{{/data.releaseTitle}}</p>
