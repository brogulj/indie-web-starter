...
<main class="mx-auto max-w-4xl p-4">
  <article class="h-entry space-y-5 rounded-2xl border border-gray-300 bg-white p-5">
    <header class="space-y-2">
      <p class="text-xs uppercase tracking-wide text-gray-500">Music Review</p>
      ...
      <p class="text-sm text-gray-600"><a class="u-url hover:underline" href="/music-reviews/{{slug}}">/music-reviews/{{slug}}</a></p>
    ...
    <section class="grid gap-4 rounded-xl border border-gray-200 bg-gray-50 p-4 sm:grid-cols-3">
      <div class="sm:col-span-2 space-y-2 text-sm text-gray-700">
        <p><strong>Artist:</strong> {{data.artistName}}</p>
        <p><strong>Release:</strong> {{data.releaseTitle}}</p>
        <p><strong>Type:</strong> {{data.releaseType}}</p>
      </div>
