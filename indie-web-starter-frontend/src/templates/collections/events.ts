...
<main class="mx-auto max-w-4xl p-4">
  <article class="h-entry space-y-5 rounded-2xl border border-gray-300 bg-white p-5">
    <header class="space-y-2">
      ...
      <p class="text-sm text-gray-600"><a class="u-url hover:underline" href="/events/{{slug}}">/events/{{slug}}</a></p>
      <p class="h-card text-sm text-gray-600"><a class="p-name u-url hover:underline" href="{{siteAuthorUrl}}">{{siteAuthorName}}</a></p>
    </header>

    <section class="grid gap-2 rounded-xl border border-gray-200 bg-gray-50 p-4 text-sm text-gray-700 sm:grid-cols-2">
      ...
      <p><strong>Outfit:</strong> {{#data.outfit}}<a href="/outfits/{{data.outfit}}" class="underline">{{data.outfit}}</a>{{/data.outfit}}{{^data.outfit}}-{{/data.outfit}}</p>
    </section>
