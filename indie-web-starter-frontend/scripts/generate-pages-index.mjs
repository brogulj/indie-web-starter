...
const pagesDir = resolve(process.cwd(), 'src/templates/pages');
const outputPath = resolve(pagesDir, 'index.ts');
const excludedFiles = new Set(['index.ts']);
...
const generateIndex = async () => {
	...
	for (const entry of entries) {
		...
		const filePath = resolve(pagesDir, entry.name);
		const fileContents = await readFile(filePath, 'utf8');
		const exportMatch = fileContents.match(/export const ([A-Za-z0-9_]+Template)\s*=/);
		if (!exportMatch) continue;

		const slug = basename(entry.name, '.ts');
		pageEntries.push({
			slug,
			fileBase: slug,
			exportName: exportMatch[1],
			requiredDataExportName: hasRequiredDataExport(fileContents) ? `${toPascalCase(slug)}RequiredData` : null,
		});
	}

	pageEntries.sort(sortBySlug);
	const entriesWithRequiredData = pageEntries.filter((entry) => entry.requiredDataExportName);
	...
	const lines = [
		...
		'',
		'export const pageTemplates: Record<string, string> = {',
		...pageEntries.map((entry) => `\t'${entry.slug}': ${entry.exportName},`),
		'};',
		'',
		'export const pageRequiredData: Record<string, unknown> = {',
	...
	console.log(`Generated pages index at ${outputPath}`);
};
...
const startWatch = () => {
	...
	watch(pagesDir, { persistent: true }, (_eventType, fileName) => {
	...
	console.log(`Watching page templates in ${pagesDir}`);
};
