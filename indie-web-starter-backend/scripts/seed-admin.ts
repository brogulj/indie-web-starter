...
async function seed() {
  ...
  try {
    ...
    const existingUser = await db
      .select()
      .from(users)
      .where(eq(users.email, 'roguljbruno@gmail.com'))
      .get()

    if (existingUser) {
      console.log('✓ Admin user already exists')
      console.log(`  Email: roguljbruno@gmail.com`)
      console.log(`  Role: ${existingUser.role}`)
      return
    }

    // Hash password using SHA-256 (same as SonicJS auth system)
    const data = 'U$vAt3kl' + 'salt-change-in-production'
